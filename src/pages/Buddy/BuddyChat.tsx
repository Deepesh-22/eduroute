  ...
  return (
    <div className="flex h-[calc(100vh-64px)] max-h-[calc(100vh-64px)] flex-1 flex-col overflow-hidden bg-[#f5f7fb] dark:bg-slate-950">
      ...
      <div className="mx-auto grid min-h-0 w-full max-w-[1440px] flex-1 grid-cols-1 overflow-hidden lg:grid-cols-[minmax(0,1fr)_350px]">
                ...
                >
                  ...
                  {message.role === 'ai' && (
                    <div className="mt-3 flex items-center gap-3 border-t border-slate-100 pt-3 text-[11px] font-bold text-slate-400 dark:border-slate-800 dark:text-slate-500">
                      ...
                      <button type="button" aria-label="Like response" className="hover:text-indigo-600 dark:hover:text-indigo-400">
                        <ThumbsUp className="h-3.5 w-3.5" />
                      </button>
                      <button type="button" aria-label="More response actions" className="hover:text-indigo-600 dark:hover:text-indigo-400">
        ...
        <aside className="min-h-0 overflow-y-auto border-t border-slate-200 bg-white/80 p-4 dark:border-slate-800 dark:bg-slate-900/80 md:p-6 lg:border-l lg:border-t-0">
          <div className="mb-4 flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500">Your cockpit</p><h2 className="mt-1 text-lg font-black tracking-tight text-slate-950 dark:text-white">Progress & prompts</h2></div><span className="flex items-center gap-1.5 text-[11px] font-bold text-emerald-600"><span className="h-2 w-2 rounded-full bg-emerald-500" /> Synced</span></div><div className="grid grid-cols-2 gap-3"><InfoCard icon={Trophy} title="Level" value={`Lv. ${progress?.level || 1}`} /><InfoCard icon={Sparkles} title="Points" value={`${progress?.points || 0} XP`} /></div>
          <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:shadow-none"><div className="flex items-center justify-between"><h2 className="flex items-center gap-2 text-sm font-black text-slate-900 dark:text-white"><Brain className="h-4 w-4 text-indigo-600" /> Skill check</h2><span className="text-xs font-bold text-indigo-600">{answeredSkills}/{SKILL_CHECK_QUESTIONS.length}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800"><div className="h-full rounded-full bg-indigo-600 transition-all" style={{ width: `${skillProgress}%` }} /></div><div className="mt-3 space-y-2">{SKILL_CHECK_QUESTIONS.map((question) => <div key={question.key} className="rounded-xl border border-slate-100 px-3 py-3 dark:border-slate-800"><p className="text-xs font-semibold leading-5 text-slate-700 dark:text-slate-300">{question.text}</p><div className="mt-2 flex gap-2"><button type="button" onClick={() => setSkillAnswers((current) => ({ ...current, [question.key]: true }))} className={`flex-1 px-2 py-1.5 text-xs ${skillAnswers[question.key] === true ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-400' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'}`}>{skillAnswers[question.key] === true && <Check className="mr-1 inline h-3 w-3" />}Yes</button><button type="button" onClick={() => setSkillAnswers((current) => ({ ...current, [question.key]: false }))} className={`flex-1 px-2 py-1.5 text-xs ${skillAnswers[question.key] === false ? 'bg-red-100 text-red-700 dark:bg-red-950/50 dark:text-red-400' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'}`}>No</button></div></div>)}</div><button type="button" onClick={() => void runSkillGapAnalyzer()} disabled={answeredSkills !== SKILL_CHECK_QUESTIONS.length} className="mt-3 w-full bg-slate-950 py-2.5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">Analyze my gap</button></section>
