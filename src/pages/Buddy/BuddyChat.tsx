import { FormEvent, useCallback, useEffect, useRef, useState, type PointerEvent as ReactPointerEvent } from 'react';
import {
  BookOpen,
  Briefcase,
  ChevronLeft,
...
export const BuddyChat = () => {
  ...
  const handleSend = async (preset?: string) => {
    ...
    setMessages((current) => [
      ...current,
      { id: Date.now(), role: 'user', text, timestamp: timestamp() },
    ]);
    setInput('');
    setError('');
    setIsTyping(true);
    ...
    try {
      ...
      const mergedMissing = [
        ...(progress?.missingSkills || []),
        ...onboard.missingSkills,
  ...
  return (
    <div className="relative flex h-[calc(100vh-5.5rem)] min-h-[520px] w-full gap-0 overflow-hidden rounded-2xl border border-slate-200/80 bg-slate-50/50 dark:border-slate-800 dark:bg-slate-950/40">
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex shrink-0 items-center justify-between gap-3 border-b border-slate-200/80 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900/80 sm:px-5">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-xs text-slate-500 dark:text-slate-400">
                Your personal study & career companion
              </p>
            </div>
          </div>
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <button
              ...
              className="rounded-xl p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-800 dark:hover:bg-slate-800 dark:hover:text-slate-200 lg:hidden"
              title={sidebarOpen ? 'Hide panel' : 'Show prompts'}
        ...
        <div ref={scrollRef} className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-5 sm:px-6">
          {messages.map((m) => (
            ...
            >
              {m.role === 'ai' && (
                <div className="mt-1 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 text-sm text-white shadow-sm">
                  🤖
                </div>
              )}
              <div className={`max-w-[min(100%,36rem)] ${m.role === 'user' ? 'order-1' : ''}`}>
                <div
                  className={`rounded-2xl px-4 py-3 text-sm leading-6 ${
                    m.role === 'user'
                      ? 'rounded-br-md bg-violet-600 text-white shadow-sm shadow-violet-200/40 dark:shadow-violet-900/30'
                      : 'rounded-bl-md border border-slate-100 bg-white text-slate-800 shadow-sm dark:border-slate-700 dark:bg-slate-800/90 dark:text-slate-100'
                  }`}
                >
              ...
              {m.role === 'user' && (
                <div className="mt-1 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-violet-600 text-xs font-bold text-white">
          ...
          {isTyping && (
            <div className="flex items-center gap-2.5">
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 text-sm text-white">
                🤖
              </div>
          ...
          {error && (
            <p className="rounded-xl bg-red-50 px-3 py-2 text-xs font-medium text-red-600 dark:bg-red-950/40 dark:text-red-300">
              {error}
            </p>
          )}
        </div>
        ...
        <form
          onSubmit={onSubmit}
          className="shrink-0 border-t border-slate-200/80 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900/80 sm:px-5"
        >
          <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-2 py-1.5 focus-within:border-violet-400 focus-within:ring-2 focus-within:ring-violet-500/15 dark:border-slate-700 dark:bg-slate-800/80 dark:focus-within:border-violet-500/50">
            <button
              type="button"
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-slate-400 hover:bg-slate-200/60 hover:text-slate-600 dark:hover:bg-slate-700 dark:hover:text-slate-300"
              aria-label="Attach"
            ...
            <button
              ...
              aria-label="Send"
              className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-violet-600 text-white shadow-sm transition hover:bg-violet-700 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400 dark:disabled:bg-slate-700 dark:disabled:text-slate-500"
            >
      ...
      <div
        ...
        className="group relative z-10 hidden w-1.5 shrink-0 cursor-col-resize touch-none bg-transparent hover:bg-violet-400/40 lg:block"
      ...
      <aside
        ...
        className={`relative hidden shrink-0 flex-col overflow-hidden border-l border-slate-200/80 bg-white transition-[width] duration-200 ease-out dark:border-slate-800 dark:bg-slate-900/60 lg:flex ${
          effectiveWidth === 0 ? 'border-l-0' : ''
        }`}
      >
        {effectiveWidth > 0 && (
                ...
                {QUICK_PROMPTS.map((item) => (
                  <li key={item.label}>
                    <button
                      ...
                      className="flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2.5 text-left text-xs font-semibold text-slate-700 transition hover:bg-white hover:text-violet-700 hover:shadow-sm dark:text-slate-200 dark:hover:bg-slate-800 dark:hover:text-violet-300"
                    >
            ...
            <section className="rounded-2xl border border-slate-100 bg-slate-50/80 p-3.5 dark:border-slate-800 dark:bg-slate-900/80">
              ...
              <ul className="space-y-1">
                ...
                {conversations.slice(0, 8).map((c) => {
                  ...
                  const title =
                    c.title ||
                    c.messages.find((m) => m.role === 'user')?.text?.slice(0, 42) ||
                    'New conversation';
      ...
      {!sidebarOpen && (
        <button
          type="button"
          onClick={toggleSidebar}
          className="absolute right-3 top-1/2 z-20 hidden -translate-y-1/2 rounded-l-xl border border-r-0 border-slate-200 bg-white px-1.5 py-3 text-slate-500 shadow-md hover:text-violet-600 dark:border-slate-700 dark:bg-slate-900 dark:hover:text-violet-400 lg:flex"
          title="Show Quick Prompts & Recent Chats"
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          />
          <div className="absolute inset-y-0 right-0 flex w-[min(320px,90vw)] flex-col gap-4 overflow-y-auto border-l border-slate-200 bg-white p-4 shadow-xl dark:border-slate-800 dark:bg-slate-900">
            <div className="flex items-center justify-between">
              <span className="text-sm font-bold text-slate-900 dark:text-white">Prompts & chats</span>
              <button
                type="button"
                onClick={() => setSidebarOpen(false)}
            ...
            <section>
              <h2 className="mb-2 flex items-center gap-1.5 text-sm font-bold">
                <Sparkles className="h-4 w-4 text-violet-500" /> Quick Prompts
              </h2>
              {QUICK_PROMPTS.map((item) => (
                <button
                  key={item.label}
                  ...
                  className="mb-1 flex w-full items-center gap-2 rounded-xl px-2 py-2.5 text-left text-xs font-semibold text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800"
                >
            ...
            <section>
              <h2 className="mb-2 flex items-center gap-1.5 text-sm font-bold">
                <BookOpen className="h-4 w-4 text-violet-500" /> Recent Chats
              </h2>
              {conversations.slice(0, 8).map((c) => (
                <button
                  key={c.id}
                  ...
                  className="mb-1 block w-full truncate rounded-xl px-2 py-2 text-left text-xs font-semibold text-slate-700 hover:bg-slate-50 dark:text-slate-200 dark:hover:bg-slate-800"
                >
                  {c.title || 'Chat'}
                </button>
              ))}
