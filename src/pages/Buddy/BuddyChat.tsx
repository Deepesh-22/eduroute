        ...
        <main className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
          <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-6 md:px-8"><div className="mx-auto max-w-4xl space-y-6">
            ...
            {messages.map((message) => <motion.div key={message.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`group flex items-end gap-3 ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={`max-w-[min(720px,88%)] rounded-3xl px-5 py-4 shadow-sm ${message.role === 'user' ? 'rounded-br-md bg-indigo-600 text-white' : 'rounded-bl-md border border-slate-200 bg-white text-slate-800'}`}><div className="mb-2 flex items-center justify-between gap-5 text-[10px] font-black uppercase tracking-widest opacity-60"><div className="flex items-center gap-2"><span>{message.role === 'user' ? 'You' : 'Buddy AI'}</span><span>•</span><span>{message.timestamp}</span></div>{message.role === 'ai' && <button type="button" onClick={() => void copyMessage(message)} aria-label="Copy Buddy response" className="opacity-0 transition-opacity group-hover:opacity-100"><Copy className="h-3.5 w-3.5" /></button></div>{message.role === 'ai' ? <BuddyMarkdown text={message.text} /> : <p className="whitespace-pre-wrap text-sm leading-7">{message.text}</p>}{message.role === 'ai' && <div className="mt-3 flex items-center gap-3 border-t border-slate-100 pt-3 text-[11px] font-bold text-slate-400"><button type="button" onClick={() => void copyMessage(message)} className="inline-flex items-center gap-1 hover:text-indigo-600">{copiedMessageId === message.id ? <Check className="h-3.5 w-3.5" /> : <Clipboard className="h-3.5 w-3.5" />}{copiedMessageId === message.id ? 'Copied' : 'Copy'}</button><button type="button" aria-label="Like response" className="hover:text-indigo-600"><ThumbsUp className="h-3.5 w-3.5" /></button><button type="button" aria-label="More response actions" className="hover:text-indigo-600"><MoreHorizontal className="h-3.5 w-3.5" /></button></div>}</div>
              {message.role === 'user' && <div className="hidden h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-indigo-100 text-indigo-700 sm:flex"><UserRound className="h-4 w-4" /></div>}
            </motion.div>)}
