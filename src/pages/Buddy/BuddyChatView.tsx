  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <ul
          role="listbox"
          className="absolute right-0 z-50 mt-2 min-w-[11rem] overflow-hidden rounded-2xl border border-violet-200/60 bg-white/95 py-1 shadow-xl shadow-violet-900/10 backdrop-blur-xl dark:border-violet-500/30 dark:bg-[#0c1230]/95 dark:shadow-black/40"
        >
  ...
  return (
    <div className="relative flex h-[calc(100vh-5.5rem)] min-h-[520px] w-full gap-0 overflow-hidden rounded-2xl border border-violet-200/50 bg-gradient-to-br from-slate-50 via-indigo-50/40 to-violet-50/50 text-slate-900 shadow-xl shadow-indigo-100/40 dark:border-indigo-500/25 dark:from-[#070b1a] dark:via-[#0c1230] dark:to-[#12082a] dark:text-slate-100 dark:shadow-indigo-950/40">
      ...
      <div className="relative z-10 flex min-h-0 min-w-0 flex-1 flex-col">
        <header className="flex shrink-0 items-center justify-between gap-3 border-b border-violet-200/40 bg-white/60 px-4 py-3 backdrop-blur-xl dark:border-white/10 dark:bg-white/5 sm:px-5">
        ...
        <div className="relative z-10 shrink-0 border-t border-violet-200/40 bg-white/70 px-3 py-3 backdrop-blur-xl dark:border-white/10 dark:bg-white/5 sm:px-5">
          ...
          <form onSubmit={onSubmit} className="flex items-center gap-2 rounded-full border border-violet-200/60 bg-white/90 px-2 py-1.5 shadow-inner focus-within:border-violet-400 focus-within:ring-2 focus-within:ring-violet-500/20 dark:border-white/15 dark:bg-white/5 dark:focus-within:border-violet-400/40">
            ...
            {voiceSupported ? (
              <button type="button" onClick={toggleListening} disabled={isTyping} className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full transition ${isListening ? 'bg-rose-500 text-white shadow-lg shadow-rose-500/40' : 'text-slate-400 hover:bg-violet-50 hover:text-violet-600 dark:hover:bg-white/10 dark:hover:text-violet-300'}`} title={isListening ? 'Stop listening' : 'Start voice input'} aria-label={isListening ? 'Stop listening' : 'Start voice input'}>
                {isListening ? <MicOff className="h-4 w-4" /> : <Mic className="h-4 w-4" />}
              </button>
            ) : (
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-slate-400" title="Voice not supported">
        ...
        {effectiveWidth > 0 && (
          <div className="flex h-full min-h-0 flex-col gap-4 p-4">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Panel</span>
              <button type="button" onClick={toggleSidebar} className="rounded-lg p-1.5 text-slate-400 transition hover:bg-violet-50 hover:text-slate-700 dark:hover:bg-white/10 dark:hover:text-slate-200" title="Collapse panel" aria-label="Collapse side panel">
                <ChevronRight className="h-4 w-4" />
              </button>
            </div>
            <section className="rounded-2xl border border-violet-200/50 bg-white/70 p-3 shadow-lg backdrop-blur-md dark:border-white/10 dark:bg-white/5">
              <div className="mb-2 flex items-center justify-between">
                ...
                <span className="text-[10px] font-semibold text-violet-500/80 dark:text-violet-300/80">Try these</span>
              </div>
              <ul className="space-y-1">
                {QUICK_PROMPTS.map((item) => {
                  ...
                  return (
                    <li key={item.label}>
                      <button type="button" onClick={() => void handleSend(item.prompt)} className="flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2.5 text-left text-xs font-semibold text-slate-700 transition hover:bg-violet-50 hover:text-violet-800 dark:text-slate-300 dark:hover:bg-violet-500/20 dark:hover:text-violet-100">
            ...
            <section className="flex min-h-0 flex-1 flex-col rounded-2xl border border-violet-200/50 bg-white/70 p-3 shadow-lg backdrop-blur-md dark:border-white/10 dark:bg-white/5">
              <div className="mb-2 flex items-center justify-between">
                <h2 className="flex items-center gap-1.5 text-sm font-bold text-slate-900 dark:text-white">
                  <MessageSquare className="h-4 w-4 text-violet-500" /> Recent
                </h2>
                <button type="button" onClick={onNewChat} className="text-[11px] font-semibold text-violet-600 hover:underline dark:text-violet-300">
                  New
                </button>
              </div>
                ...
                {conversations.map((c) => {
                  ...
                  return (
                    <li key={c.id} className="group flex items-stretch gap-0.5">
                      <button type="button" onClick={() => onSelectChat(c.id)} className={`flex min-w-0 flex-1 items-start gap-2.5 rounded-xl px-2.5 py-2.5 text-left transition ${active ? 'bg-gradient-to-r from-violet-600 to-indigo-600 text-white shadow-md shadow-violet-500/25' : 'text-slate-700 hover:bg-violet-50 hover:text-violet-900 dark:text-slate-300 dark:hover:bg-white/10 dark:hover:text-white'}`}>
                      ...
                      <button type="button" onClick={(e) => onDeleteChat(c.id, e)} className="mt-1 shrink-0 self-start rounded-lg p-1.5 text-slate-400 opacity-0 transition group-hover:opacity-100 hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-500/20 dark:hover:text-rose-300" title="Delete chat" aria-label="Delete chat">
