...
/** Shared constellation / network backdrop — used only on the top Internships hero */
...
export const Internships = () => {
  ...
  const scored = useMemo(
    () =>
      allJobs.map((job) => {
        ...
        const general =
          typeof job.generalMatch === 'number' ? job.generalMatch : stableGeneralMatch(job.id);
        // Unique baseline always; skill profile can only raise the score (never collapse all to ~13%)
        const matchScore = skillProfile.hasProfile
          ? Math.min(99, Math.max(general, profileScore))
          : general;
  ...
  return (
    <div className="relative mx-auto max-w-7xl flex-1 overflow-hidden p-4 md:p-8">
      ...
      >
        <div className="pointer-events-none absolute inset-0 opacity-40 dark:opacity-100" aria-hidden>
          <NetworkBackdrop />
        </div>
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-white/90 via-white/70 to-transparent dark:from-[#0b1224] dark:via-[#0b1224]/85 dark:to-transparent" aria-hidden />
