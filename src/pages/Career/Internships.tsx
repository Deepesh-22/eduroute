      ...
      <div className="flex flex-col gap-8 lg:flex-row">
        ...
        <div className="flex flex-col items-end justify-between gap-6 border-t border-slate-50 pt-6 dark:border-slate-800 lg:border-l lg:border-t-0 lg:pl-8 lg:pt-0">
          ...
          <div className="flex w-full flex-col items-stretch gap-2 lg:w-auto lg:items-end">
            ...
            <Link
              to={`/companies/${job.id}`}
              className="text-center text-xs font-bold text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400"
            >
              View details
            </Link>
          </div>
...
function matchesSectorFilter(job: Internship, filter: FilterKey): boolean {
  ...
  if (filter === 'remote') return /remote/i.test(job.location);
  if (filter === 'frontend') return /frontend|react|ui\/ux|design/i.test(`${job.role} ${job.tags.join(' ')}`);
  if (filter === 'backend') return /backend|node|api|devops|cloud/i.test(`${job.role} ${job.tags.join(' ')}`);
  if (filter === 'cyber') return job.sector === 'cybersecurity' || /security|soc|threat/i.test(job.role);
  if (filter === 'data') return job.sector === 'data' || /data|analyst|ml|bi/i.test(job.role);
  if (filter === 'design') return job.sector === 'design' || /design|ux|ui/i.test(job.role);
  return true;
}
