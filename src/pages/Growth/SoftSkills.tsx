      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        {SOFT_SKILLS.map((skill, i) => (
          ...
          >
            ...
            <h3 className="text-xl font-black text-slate-900 dark:text-white mb-2">{skill.title}</h3>
            <div className="flex items-center gap-3 text-xs font-bold text-slate-400 mb-6">
              <span className="inline-flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> {skill.duration}</span>
      ...
      <div className="bg-slate-900 rounded-[40px] p-8 md:p-12 text-white relative overflow-hidden">
        <div className="max-w-xl">
          ...
          <p className="text-slate-400 mb-8 font-medium">
            Learn how to present your technical projects as compelling stories. This module includes a mock interview with Buddy AI.
          </p>
          ...
          <button
            type="button"
            onClick={() => navigate('/buddy')}
            className="px-10 py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-lg shadow-indigo-900 hover:bg-indigo-700 transition-all flex items-center gap-2"
          >
            Start Module <ChevronRight className="h-5 w-5" />
          </button>
        </div>
        <div className="hidden lg:block absolute -right-20 -bottom-20 w-96 h-96 bg-indigo-600/20 blur-[100px]" />
        <div className="hidden lg:block absolute top-1/2 -translate-y-1/2 right-12 w-64 h-64 border-4 border-white/5 rounded-full" />
