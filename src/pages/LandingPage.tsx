...
const STATS = [
  { target: 45000, suffix: 'K+', divisor: 1000, label: 'Active Learners', icon: Users, color: 'bg-violet-100 text-violet-600 dark:bg-violet-500/20 dark:text-violet-300' },
  { target: 80, suffix: '+', divisor: 1, label: 'Roadmaps', icon: Map, color: 'bg-emerald-100 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-300' },
  { target: 500, suffix: '+', divisor: 1, label: 'Job & Internship Opportunities', icon: Briefcase, color: 'bg-blue-100 text-blue-600 dark:bg-blue-500/20 dark:text-blue-300' },
  { target: 120, suffix: '+', divisor: 1, label: 'Upcoming Hackathons', icon: Trophy, color: 'bg-amber-100 text-amber-600 dark:bg-amber-500/20 dark:text-amber-300' },
];

const HOW_IT_WORKS = [
  { step: '01', title: 'Sign up', desc: 'Create your free account in under a minute.' },
  { step: '02', title: 'Skill quiz', desc: 'Pick interests and mark skill gaps in a short Yes/No flow.' },
  { step: '03', title: 'Get your path', desc: 'Unlock roadmaps, internships, and match scores built for you.' },
];
...
const COMPANY_LINKS = [
  ...
  { label: 'Our Mission', href: '#journey' },
...
function CountUpStat({ target, suffix, divisor, label, icon: Icon, color }: (typeof STATS)[number]) {
  ...
  const display = divisor > 1 ? `${Math.round(val / divisor)}${suffix}` : `${val}${suffix}`;

  return (
    <div ref={ref} className="flex items-center gap-3 px-2 py-2">
      <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${color}`}>
        <Icon className="h-5 w-5" />
      </div>
      <div>
        <div className="text-lg font-extrabold text-slate-900 dark:text-white">{display}</div>
...
export const LandingPage = () => {
  ...
  return (
    <div id="home" className="min-h-screen bg-white text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      ...
      <section id="how-it-works" className="relative z-10 py-14 sm:py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mb-10 text-center">
            <p className="mb-2 text-xs font-bold uppercase tracking-widest text-violet-600 dark:text-violet-400">How it works</p>
            <h2 className="text-2xl font-extrabold tracking-tight text-slate-900 sm:text-3xl dark:text-white">From signup to your first path</h2>
            <p className="mx-auto mt-2 max-w-lg text-sm text-slate-500 dark:text-slate-400">Three steps. No noise. Built for students who want skills that hire.</p>
          </div>
          <div className="grid gap-4 sm:grid-cols-3">
            {HOW_IT_WORKS.map((item) => (
              <div
                key={item.step}
                className="rounded-2xl border border-slate-200/90 bg-white p-6 shadow-sm transition hover:border-violet-200 hover:shadow-md dark:border-slate-800 dark:bg-slate-900/80 dark:hover:border-violet-500/40"
              >
                <div className="mb-3 text-xs font-black tracking-widest text-violet-600 dark:text-violet-400">{item.step}</div>
      ...
      <footer id="contact" className="border-t border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-[#0a0a0f]">
        <div className="mx-auto max-w-7xl px-4 pt-10 sm:px-6 lg:px-8">
          <div className="select-none overflow-hidden text-center" aria-hidden>
            <span
              ...
              style={{ color: 'transparent', WebkitTextStroke: '2px rgba(109, 40, 217, 0.55)' }}
            >
            ...
            <span
              ...
              style={{ color: 'transparent', WebkitTextStroke: '2px rgba(167, 139, 250, 0.55)' }}
            >
              EDUROUTE
            </span>
          </div>
          ...
          <div className="mt-6 grid gap-10 pb-8 sm:grid-cols-2 lg:grid-cols-4 lg:gap-8">
            <div>
              <div className="flex items-center gap-2.5">
                <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-violet-600 to-indigo-600 text-sm font-black text-white shadow-md">E</span>
                <span className="text-lg font-extrabold tracking-tight text-slate-900 dark:text-white">
                  EDU<span className="text-violet-600 dark:text-violet-400">ROUTE</span>
                </span>
              </div>
              ...
              <div className="mt-5 flex flex-wrap items-center gap-3">
                <a href={instagramUrl} target="_blank" rel="noreferrer" aria-label="Instagram" className="flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:border-pink-300 hover:text-pink-600 dark:border-slate-700 dark:text-slate-400 dark:hover:border-pink-500/50 dark:hover:text-pink-400">
                  <Instagram className="h-4 w-4" />
                </a>
                <a href={whatsappUrl} target="_blank" rel="noreferrer" aria-label="WhatsApp" className="flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:border-emerald-300 hover:text-emerald-600 dark:border-slate-700 dark:text-slate-400 dark:hover:border-emerald-500/50 dark:hover:text-emerald-400">
                  <MessageCircle className="h-4 w-4" />
                </a>
                <a href={`mailto:${supportEmail}`} aria-label="Email" className="flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:border-violet-300 hover:text-violet-600 dark:border-slate-700 dark:text-slate-400 dark:hover:border-violet-500/50 dark:hover:text-violet-300">
            ...
            <div>
              <h3 className="mb-4 text-xs font-bold uppercase tracking-widest text-slate-900 dark:text-white">Quick Links</h3>
              <ul className="space-y-2.5">
                {QUICK_LINKS.map((item) =>
                  item.href.startsWith('/') ? (
                    <li key={item.label}>
                      <Link to={item.href} className="text-sm text-slate-500 transition hover:text-violet-600 dark:text-slate-400 dark:hover:text-violet-300">{item.label}</Link>
                    </li>
                  ) : (
                    <li key={item.label}>
                      <a href={item.href} className="text-sm text-slate-500 transition hover:text-violet-600 dark:text-slate-400 dark:hover:text-violet-300">{item.label}</a>
                    </li>
                  ),
                )}
              </ul>
            ...
            <div>
              <h3 className="mb-4 text-xs font-bold uppercase tracking-widest text-slate-900 dark:text-white">Company</h3>
              <ul className="space-y-2.5">
            ...
            <div>
              <h3 className="mb-4 text-xs font-bold uppercase tracking-widest text-slate-900 dark:text-white">Stay Updated</h3>
              <p className="mb-3 text-xs leading-5 text-slate-500 dark:text-slate-400">Get the latest roadmaps, opportunities and hackathon alerts.</p>
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  setNewsletterEmail('');
              ...
              >
                <input
                  ...
                  required
                />
                <button type="submit" className="rounded-lg bg-violet-600 px-3 py-2 text-sm font-bold text-white transition hover:bg-violet-700">Subscribe</button>
              </form>
            </div>
          </div>
