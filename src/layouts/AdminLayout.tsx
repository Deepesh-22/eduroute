  ...
  return (
    <div className="flex min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)]">
      <aside className="hidden md:flex w-64 flex-col bg-slate-950 text-slate-100">
        ...
        <div className="p-4 border-t border-slate-800 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            <div className="h-9 w-9 rounded-full bg-indigo-600 flex items-center justify-center font-bold shrink-0">
          ...
          <button type="button" onClick={handleLogout} className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800" title="Logout">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-20 border-b border-[var(--border-default)] bg-[var(--surface-nav)] backdrop-blur-md px-4 md:px-6 py-3 flex items-center gap-3">
          ...
          <div className="hidden sm:flex items-center gap-2 rounded-xl border border-[var(--border-default)] bg-[var(--surface-input)] px-3 py-1.5">
            <div className="h-8 w-8 rounded-full bg-indigo-600 text-white flex items-center justify-center text-sm font-bold">
              {(authUser?.name || 'A').charAt(0).toUpperCase()}
            </div>
            <span className="text-sm font-semibold">{authUser?.name || 'Admin'}</span>
          </div>
        </header>
